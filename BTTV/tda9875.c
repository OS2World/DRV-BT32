/* $Id: tda9875.c,v 1.12 2004/01/29 09:10:17 smilcke Exp $ */
...
static int tda9875_command(struct i2c_client *client,unsigned int cmd,void *arg);
...
static int tda9875_write(struct i2c_client *client,int subaddr,unsigned char val)
{
 ...
 CPK(printk("In tda9875_write\n"));
 CPK(printk("Writing %d 0x%x\n", subaddr, val));
 buffer[0]=subaddr;
 buffer[1]=val;
 if(2!=i2c_master_send(client,(char*)buffer,2))
 {
  CPK(printk(KERN_WARNING "tda9875: I/O error, trying (write %d 0x%x)\n"
             ,subaddr,val));
  return -1;
 }
 return 0;
...
static int tda9875_read(struct i2c_client *client)
{
 ...
 CPK(printk("In tda9875_read\n"));
 if(1!=i2c_master_recv(client,&buffer,1))
 {
  CPK(printk(KERN_WARNING "tda9875: I/O error, trying (read)\n"));
  return -1;
 }
 CPK(printk("Read 0x%02x\n",buffer));
 return buffer;
}
#endif
...
static int i2c_read_register(struct i2c_adapter* adap,int addr,int reg)
{
 ...
 {
  CPK(printk(KERN_WARNING "tda9875: I/O error (read2)\n"));
  return -1;
 }
 CPK(dprintk("tda9875: chip_read2: reg%d=0x%x\n",reg,read[0]));
 return (int)(read[0]);
}
...
{
 struct tda9875 *tda = (struct tda9875 *)i2c_get_clientdata(client);
 unsigned char a;
 CPK(dprintk(KERN_DEBUG "tda9875_set(%04x,%04x,%04x,%04x)\n",
...
{
 struct tda9875 *t = (struct tda9875*)i2c_get_clientdata(client);
 CPK(dprintk("In tda9875_init\n"));
...
static int tda9875_checkit(struct i2c_adapter* adap,int addr)
{
 ...
 { // tda9875 and tda9875A
  CPK(printk("tda9875: TDA9875%s Rev.%d detected at 0x%x\n"
             ,dic==0?"":"A",rev,addr<<1));
  return 1;
 }
 CPK(printk("tda9875: no such chip at 0x%x (dic=0x%x rev=0x%x)\n",addr<<1,dic,rev));
 return 0;
}
                          ...
                          ,unsigned short flags,int kind)
{
 ...
 CPK(printk("In tda9875_attach\n"));
 t = (struct tda9875 *)kmalloc(sizeof *t,GFP_KERNEL);
 if(!t)
  return -ENOMEM;
 memset(t,0,sizeof *t);
 client = &t->c;
 ...
 CPK(printk(KERN_INFO "tda9875: init\n"));
...
static int tda9875_command(struct i2c_client *client,unsigned int cmd,void *arg)
{
 ...
 switch(cmd)
 {
  ...
  case VIDIOCGAUDIO:
  {
   ...
   int left,right;
   CPK(printk("VIDIOCGAUDIO\n"));
   va->flags |= VIDEO_AUDIO_VOLUME | VIDEO_AUDIO_BASS | VIDEO_AUDIO_TREBLE;
   // min is -84 max is 24
   left = (t->lvol+84)*606;
   right = (t->rvol+84)*606;
  ...
  case VIDIOCSAUDIO:
  {
   struct video_audio *va = (struct video_audio*)arg;
   int left,right;
   CPK(printk("VIDIOCSAUDIO...\n"));
   left = (MIN(65536 - va->balance,32768) * va->volume) / 32768;
   right = (MIN(va->balance,32768) * va->volume) / 32768;
...
//--------------------------- tda9875_cleanup_module ---------------------------
void tda9875_cleanup_module(void)
{
