/* $Id: bttv-driver.c,v 1.45 2004/01/29 09:10:12 smilcke Exp $ */
...
// From bttvcrds.c (SM)
extern unsigned int triton1;
extern unsigned int vsfx;
extern unsigned int latency;
extern unsigned int no_overlay;
extern unsigned int *card;
extern unsigned int *pll;
extern unsigned int *tuner;
extern unsigned int autoload;
extern unsigned int gpiomask;
extern unsigned int audioall;
extern unsigned int *audiomux;


MODULE_PARM_LIST_HEAD(bttvdrv_parms)
MODULE_PARM_A(radio,"1-4i")
...
MODULE_PARM(card,"1-8i")
MODULE_PARM(pll,"1-8i")
MODULE_PARM(tuner,"1-8i")
MODULE_PARM(autoload,"i")
MODULE_PARM(gpiomask,"i")
MODULE_PARM(audioall,"i")
MODULE_PARM(audiomux,"1-5i")
MODULE_PARM_LIST_TAIL(bttvdrv_parms)
...
//struct video_device bttv_template;
//struct video_device vbi_template;
//struct video_device radio_template;

#ifdef TARGET_OS2
extern unsigned long hrmBttvDriver;
extern unsigned long hrmBttvAdapter;
static struct lxrm_driver rmDriver={0,0};
static DEVICESTRUCT rmBttvDeviceStruct=
{
 (unsigned char*)"BTTV",
...
static struct lxrm_device rmBttvDevice={0,0,0,&rmBttvDeviceStruct};
static struct lxrm_resource rmIrqResource={0,0,0,0};
static struct lxrm_resource rmMemResource={0,0,0,0};
#endif
...
// kva=uvirt_to_kva(pgd_offset_k(va),va);
 // Correct ? (SM)
 return (unsigned long)virt_to_bus((void *)adr);
}
...
/*
 ...
 kva=uvirt_to_kva(pgd_offset_k(va),va);
 ret=__pa(kva);
 CPK(printk("kv2pa(%lx-->%lx)",adr,ret));
 return ret;
*/
...
{
 if(!btv->fbuffer)
  btv->fbuffer=(char *)rvmalloc(gbuffers*gbufsize);
 else
  CPK(printk(KERN_ERR "bttv%d: Double alloc of fbuffer!\n",btv->nr));
 if(!btv->fbuffer)
  return -ENOBUFS;
 return 0;
}
...
void bttv_gpio_tracking(struct bttv *btv,char *comment)
{
 ...
 CPK(printk(KERN_DEBUG "bttv%d: gpio: en=%08x, out=%08x in=%08x [%s]\n"
...
static void audio(struct bttv *btv,int mode)
{
 ...
 if ((btv->audio&AUDIO_MUTE))
  mode=AUDIO_OFF;
 if ((mode == AUDIO_TUNER) && (btv->radio_user))
  mode = AUDIO_RADIO;
 if (bttv_tvcards[btv->type].gpiomask)
...
static void set_pll(struct bttv *btv)
{
 ...
 {
  CPK(dprintk("bttv%d: PLL: no change required\n",btv->nr));
  return;
 }
 if (btv->pll.pll_ifreq == btv->pll.pll_ofreq)
 {
  ...
  if (btv->pll.pll_current == 0)
   return;
  CPK(vprintk(KERN_INFO "bttv%d: PLL can sleep, using XTAL (%d).\n",
 ...
 CPK(vprintk(KERN_INFO "bttv%d: PLL: %d => %d ",btv->nr,
...
{
 CPK(dprintk("bttv%d: bt848_muxsel %d\n",btv->nr,input));
 if (bttv_tvcards[btv->type].muxsel[input] < 0)
 {
  CPK(dprintk("bttv%d: digital ccir muxsel\n", btv->nr));
  btv->channel = input;
  return;
 }
...
void bt848A_set_timing(struct bttv *btv)
{
 ...
 {
  CPK(dprintk("bttv%d: load digital timing table (table_idx=%d)\n",
         btv->nr,table_idx));
...
static void bttv_set_norm(struct bttv *btv,int new_norm)
{
     ...
     bttv_tvcards[btv->type].digital_mode == DIGITAL_MODE_CAMERA)
 {
  ...
  new_norm=7;
  CPK(dprintk("bttv%d: set_norm fix-up digital: %d\n",
         btv->nr, new_norm));
 }
...
static void make_vbitab(struct bttv *btv)
{
 ...
 if (bttv_debug > 1)
  CPK(printk("bttv%d: vbi1: po=%08lx pe=%08lx\n",
 ...
 *(pe++)=cpu_to_le32(virt_to_bus(btv->risc_jmp+10));
 if (bttv_debug > 1)
  CPK(printk("bttv%d: vbi2: po=%08lx pe=%08lx\n",
             btv->nr,virt_to_bus(po), virt_to_bus(pe)));
}
                         ...
                         ,u16 width,u16 height,u16 fmt)
{
 ...
 u32 shift, csize;
 if (bttv_debug > 1)
  CPK(printk("bttv%d: prisc1: ro=%08lx re=%08lx\n",
 ...
 *(re++)=cpu_to_le32(btv->bus_vbi_odd);
 if (bttv_debug > 1)
  CPK(printk("bttv%d: prisc2: ro=%08lx re=%08lx\n",
             btv->nr,virt_to_bus(ro), virt_to_bus(re)));
 return 0;
}
                         ...
                         ,u16 width,u16 height,u16 palette)
{
 ...
 if (bttv_debug > 1)
  CPK(printk("bttv%d: vrisc1: ro=%08lx re=%08lx\n",
             btv->nr,virt_to_bus(ro), virt_to_bus(re)));
 inter = (height>tvnorms[btv->win.norm].sheight/2) ? 1 : 0;
 ...
 *(re++)=cpu_to_le32(btv->bus_vbi_odd);
 if (bttv_debug > 1)
  CPK(printk("bttv%d: vrisc2: ro=%08lx re=%08lx\n",
             btv->nr,virt_to_bus(ro), virt_to_bus(re)));
 return 0;
}
...
static void make_clip_tab(struct bttv *btv, struct video_clip *cr, int ncr)
{
 ...
 u32 adr;
 int i;
 unsigned char *clipmap, *clipline;
 unsigned char cbit, lastbit, outofmem;
 ...
 if (bttv_debug > 1)
  CPK(printk("bttv%d: clip1: pal=%d size=%dx%d, bpl=%d bpp=%d\n",
 ...
 if (bttv_debug)
  CPK(printk("bttv%d: clip: ro=%08lx re=%08lx\n",
         btv->nr,virt_to_bus(ro), virt_to_bus(re)));

 if ((clipmap=(unsigned char*)vmalloc(VIDEO_CLIPMAP_SIZE))==0)
 {
 ...
 *(re++)=cpu_to_le32(btv->bus_vbi_odd);
 if (bttv_debug > 1)
  CPK(printk("bttv%d: clip2: pal=%d size=%dx%d, bpl=%d bpp=%d\n",
         btv->nr,btv->picture.palette,width,height,bpl,bpp));
}
                                   ...
                                   ,int odd,int width,int height)
{
 ...
 if (bttv_tvcards[btv->type].muxsel[btv->channel] < 0)
 {
  CPK(dprintk("bttv%d: DIGITAL_MODE_VIDEO override width\n",btv->nr));
  swidth       = 720;
  totalwidth   = 858;
  scaledtwidth = 858;
 }
 xsf = (width*scaledtwidth)/swidth;
 hscale = (u16)(((totalwidth*4096UL)/xsf-4096));
 hdelay =  tvn->hdelayx1;
 hdelay =  (hdelay*width)/swidth;
 hdelay &= 0x3fe;
 sr=((tvn->sheight>>inter)*512)/height-512;
 vscale=(u16)((0x10000UL-sr)&0x1fff);
...
static void bt848_set_winsize(struct bttv *btv)
{
 ...
 if (btv->picture.palette > 0 && btv->picture.palette <= VIDEO_PALETTE_YUV422)
 {
  ...
  format = (unsigned short)(palette2fmt[btv->picture.palette]);
 }
 else
 {
...
static int vgrab(struct bttv *btv,struct video_mmap *mp)
{
 ...
 if(mp->frame >= gbuffers /*|| mp->frame < 0 (SM) */)
  return -EINVAL;
  ...
  */
 btv->gbuf[mp->frame].stat    = GBUFFER_GRABBING;  // (SM): In Original code below
 vbuf=(unsigned int *)(btv->fbuffer+gbufsize*mp->frame);
 ro=btv->gbuf[mp->frame].risc;
 re=ro+2048;
 make_vrisctab(btv, ro, re, vbuf, mp->width, mp->height, (u16)mp->format);
 if (bttv_debug)
  CPK(printk("bttv%d: cap vgrab: queue %d (%d:%dx%d)\n",
         btv->nr,mp->frame,mp->format,mp->width,mp->height));
        spin_lock_irqsave(&btv->s_lock, flags);
// btv->gbuf[mp->frame].stat    = GBUFFER_GRABBING;  // (SM): In Original code above
 btv->gbuf[mp->frame].fmt     = (u16)(palette2fmt[mp->format]);
...
//--------------------------------- bttv_write ---------------------------------
static long bttv_write(struct video_device *v,const char *buf
...
//--------------------------------- bttv_read ----------------------------------
static long bttv_read(struct video_device *v,char *buf
                      ,unsigned long count,int nonblock)
{
 return -EINVAL;
}
...
void bt848_set_risc_jmps(struct bttv *btv,int flags);
...
static void bt848_restart(struct bttv *btv)
{
 unsigned long irq_flags;
 if (bttv_verbose)
  CPK(printk("bttv%d: resetting chip\n",btv->nr));
  ...
  case VIDIOCGCHAN:
   {
    ...
    else if (channel==btv->svhs) // NEW (SM)
  ...
  case VIDIOCSWIN:
   {
    ...
    btv->win.x=vw.x;
    btv->win.y=vw.y;
    btv->win.width=(ushort)vw.width;
    btv->win.height=(ushort)vw.height;
    spin_lock_irqsave(&btv->s_lock, irq_flags);
    bt848_set_risc_jmps(btv,0);
    bt848_set_winsize(btv);
    spin_unlock_irqrestore(&btv->s_lock, irq_flags);
    ...
    if(vw.clipcount<0)
    {
     if((vcp=(struct video_clip*)vmalloc(VIDEO_CLIPMAP_SIZE))==NULL)
     {
      up(&btv->lock);
      return -ENOMEM;
     }
    ...
    {
     if((vcp=(struct video_clip*)vmalloc(sizeof(struct video_clip)*
        (vw.clipcount))) == NULL)
     {
      up(&btv->lock);
      return -ENOMEM;
...
#endif
    if (bttv_debug)
     CPK(printk("Display at %p is %d by %d, bytedepth %d, bpl %d\n",
  ...
  case VIDIOCSFREQ:
   {
    ...
    if(copy_from_user(&v,arg,sizeof(v)))
     return -EFAULT;
    btv->win.freq=(ushort)v;
    bttv_call_i2c_clients(btv,cmd,&v);
#if 1
    if(btv->radio_user && btv->has_matchbox)
     tea5757_set_freq(btv,(unsigned short)v);
#endif
    return 0;
   }
  case VIDIOCGAUDIO:
  ...
  case VIDIOCSYNC:
   {
    ...
    switch(btv->gbuf[i].stat)
    {
     ...
     case GBUFFER_GRABBING:
      ...
      {
       CPK(printk("bttv%d: cap sync: sleep on %d\n",btv->nr,i));
       schedule();
       c++;
       if(c>100)
        btv->gbuf[i].stat=GBUFFER_ERROR;
     ...
     case GBUFFER_ERROR:
      ...
      CPK(printk("bttv%d: cap sync: buffer %d, retval %d\n",btv->nr,i,ret));
      btv->gbuf[i].stat=GBUFFER_UNUSED;
    }
    if(btv->needs_restart)
    {
...
{
 // This was the original code from bttv driver
 /*
 ...
 return 0;
 */
 // In OS/2 this is completely different
 unsigned long start=(unsigned long)adr;
 unsigned long page,pos;
 if(size>gbuffers*gbufsize)
  return -EINVAL;
 ...
 // In OS/2 the vmalloc'ed buffer is contiguous, so whe don't need to
 // remap it page per page
 if(remap_page_range((unsigned long)adr,(unsigned long)btv->fbuffer,size,0))
  return -EAGAIN;
 return 0;
}
  ...
  case VIDIOCGTUNER:
   {
    ...
    v.rangelow=(int)(65*freq_stepping);
    v.rangehigh=(int)(108*freq_stepping);
    v.flags=0; // XXX
    v.mode=0;  // XXX
    bttv_call_i2c_clients(btv,cmd,&v);
    if(copy_to_user(arg,&v,sizeof(v)))
...
static void bt848_set_risc_jmps(struct bttv *btv,int flags)
{
 ...
 if (bttv_debug > 1)
  CPK(printk("bttv%d: set_risc_jmp %08lx:",
 ...
 {
  if (bttv_debug > 1)
   CPK(printk(" ev=%08lx",virt_to_bus(btv->vbi_odd)));
 ...
 {
  if (bttv_debug > 1)
   CPK(printk(" e%d=%08x",btv->gq_grab,btv->risc_cap_odd));
  flags |= 3;
 ...
 {
  if (bttv_debug > 1)
   CPK(printk(" eo=%08lx",virt_to_bus(btv->risc_scr_odd)));
 ...
 {
  if (bttv_debug > 1)
   CPK(printk(" ov=%08lx",virt_to_bus(btv->vbi_even)));
 ...
 {
  if (bttv_debug > 1)
   CPK(printk(" o%d=%08x",btv->gq_grab,btv->risc_cap_even));
  flags |= 3;
 ...
 {
  if (bttv_debug > 1)
   CPK(printk(" oo=%08lx",virt_to_bus(btv->risc_scr_even)));
...
static int __devinit init_video_dev(struct bttv *btv)
{
 ...
 if (video_register_device(&btv->video_dev,VFL_TYPE_GRABBER,video_nr)<0)
  return -1;
 CPK(printk(KERN_INFO "bttv%d: registered device video%d\n",
 ...
 CPK(printk(KERN_INFO "bttv%d: registered device vbi%d\n",
 ...
 if (btv->has_radio)
 {
  ...
  CPK(printk(KERN_INFO "bttv%d: registered device radio%d\n",
...
static int __devinit init_bt848(struct bttv *btv)
{
 ...
 if(0==(btv->risc_scr_odd=(unsigned int *)kmalloc(RISCMEM_LEN/2,GFP_KERNEL)))
  return -1;
 if(0==(btv->risc_scr_even=(unsigned int *)kmalloc(RISCMEM_LEN/2,GFP_KERNEL)))
  return -1;
 if(0==(btv->risc_jmp=(unsigned int *)kmalloc(2048,GFP_KERNEL)))
  return -1;
 ...
 btv->vbibuf=(unsigned char *)kmalloc(VBIBUF_SIZE,GFP_KERNEL);// (SM):vmalloc_32
 if(!btv->vbibuf)
  return -1;
 if(0==(btv->gbuf=(struct bttv_gbuf*)kmalloc(sizeof(struct bttv_gbuf)*gbuffers,GFP_KERNEL)))
  return -1;
 for(j=0;j<gbuffers;j++)
  if(0==(btv->gbuf[j].risc=(unsigned int*)kmalloc(16384,GFP_KERNEL)))
   return -1;
 memset(btv->vbibuf,0,VBIBUF_SIZE); // We don't want to return random memory
 btv->fbuffer=0;
...
static void bttv_irq(int irq,void *dev_id,struct pt_regs *regs)
{
 ...
 while(1)
 {
  ...
  if(!claimed)
  {
   claimed=1;
   claim_irq(irq);
//   eoi_irq(irq,IRQ_FLAG_IMMEDIATE_EOI | IRQ_FLAG_CLI_BEFORE_EOI);
//   eoi_irq(irq,IRQ_FLAG_CLI_BEFORE_EOI);
   eoi_irq(irq,0);
  }
#endif
  ...
  if(astat&BT848_INT_RISCI)
  {
   ...
   if(stat&(2<<28) && btv->gq_grab != -1)
   {
    ...
    else
    {
     ...
     bt848_set_risc_jmps(btv,-1);
     bt848_set_geo(btv);
     btwrite(btv->fb_color_ctl | BT848_COLOR_CTL_GAMMA,BT848_COLOR_CTL);
    }
    spin_unlock(&btv->s_lock);
    wake_up_interruptible(&btv->capq);
    break;
...
static void __devexit bttv_remove(struct pci_dev *pci_dev)
{
 ...
 if(btv->vbibuf)
  kfree((void *)btv->vbibuf); // vfree (SM) !!
 free_irq(btv->dev->irq,btv,&rmIrqResource);
 if(btv->bt848_mem)
 ...
 release_mem_region(pci_resource_start(btv->dev,0)
                    ,pci_resource_len(btv->dev,0),&rmMemResource);
...
static int __devinit bttv_probe(struct pci_dev *dev,const struct pci_device_id *pci_id)
{
...
#endif
#ifdef TARGET_OS2
 char bttvname[50]="BTTV";
 dev->rm_device=&rmBttvDevice;
 rmBttvDevice.hDriver=hrmBttvDriver;
 rmBttvDeviceStruct.DevDescriptName=(unsigned char*)"<UNSET>";
 if(0==hrmBttvAdapter)
 {
  ADAPTERSTRUCT AdapterStruct;
  sprintf(&(bttvname[4]),"%d",bttv_num+1);
  AdapterStruct.AdaptDescriptName=(unsigned char*)bttvname;
  AdapterStruct.AdaptFlags=AS_NO16MB_ADDRESS_LIMIT;
  AdapterStruct.BaseType=AS_BASE_MMEDIA;
  ...
  AdapterStruct.pAdjunctList=0;
  RM32CreateAdapter(rmBttvDevice.hDriver,&hrmBttvAdapter,&AdapterStruct,0,0);
  rmIrqResource.hDriver=hrmBttvDriver;
  rmIrqResource.hAdapterOrDevice=hrmBttvAdapter;
  rmMemResource.hDriver=hrmBttvDriver;
  rmMemResource.hAdapterOrDevice=hrmBttvAdapter;
 }
 rmBttvDevice.hAdapter=hrmBttvAdapter;
#endif
 CPK(printk(KERN_INFO "bttv: Bt8xx card found (%d).\n", bttv_num));
 btv=&bttvs[bttv_num];
 btv->dev=dev;
 btv->nr=bttv_num;
 btv->bt848_mem=0;
 ...
 CPK(printk(KERN_INFO "bttv%d: Bt%d (rev %d) at %02x:%02x.%x, ",
        bttv_num,btv->id, btv->revision, dev->bus->number,
        PCI_SLOT(dev->devfn),PCI_FUNC(dev->devfn)));
 CPK(printk("irq: %d, latency: %d, memory: 0x%lx\n",
        btv->irq, lat, btv->bt848_adr));
 bttv_idcard(btv);
#ifdef TARGET_OS2
 rmBttvDeviceStruct.DevDescriptName=(unsigned char*)bttvs[bttv_num].video_dev.name;
#endif
 btv->bt848_mem=(unsigned char *)ioremap(btv->bt848_adr,0x1000);
 if(!btv->bt848_mem)
  goto fail1;
 // clear interrupt mask
 btwrite(0,BT848_INT_MASK);
 result=request_irq(btv->dev->irq,bttv_irq,SA_SHIRQ|SA_INTERRUPT
                     ,"bttv",(void *)btv,&rmIrqResource);
 if(result==-EINVAL)
 {
  CPK(printk(KERN_ERR "bttv%d: Bad irq number or handler\n",bttv_num));
  goto fail1;
 }
 if(result==-EBUSY)
 {
  CPK(printk(KERN_ERR "bttv%d: IRQ %d busy, change your PnP config in BIOS\n"
             ,bttv_num,btv->irq));
  goto fail1;
 }
 if(result<0)
...
static struct pci_device_id bttv_pci_tbl[] __devinitdata=
{
 ...
 {PCI_VENDOR_ID_BROOKTREE,PCI_DEVICE_ID_BT849,PCI_ANY_ID,PCI_ANY_ID,0,0,0},
 {PCI_VENDOR_ID_BROOKTREE,PCI_DEVICE_ID_BT878,PCI_ANY_ID,PCI_ANY_ID,0,0,0},
 {PCI_VENDOR_ID_BROOKTREE,PCI_DEVICE_ID_BT879,PCI_ANY_ID,PCI_ANY_ID,0,0,0},
 {0,}
};
...
int bttv_init_module(void)
{
 int rc;
 bttv_num=0;
 CPK(printk(KERN_INFO "bttv: driver version %d.%d.%d loaded\n"
 ...
 if (bttv_verbose)
  CPK(printk(KERN_INFO "bttv: using %d buffers with %dk (%dk total) for capture\n",
         gbuffers,gbufsize/1024,gbuffers*gbufsize/1024));
 bttv_pci_driver.name="bttv";
 bttv_pci_driver.id_table=bttv_pci_tbl;
 bttv_pci_driver.probe=bttv_probe;
 bttv_pci_driver.remove=bttv_remove;
 bttv_check_chipset();
#ifdef TARGET_OS2
 bttv_pci_driver.rm_driver=&rmDriver;
 rmDriver.hDriver=hrmBttvDriver;
 rmBttvDevice.hDriver=hrmBttvDriver;
#endif
 ...
 if(-ENODEV==rc)
 {
  ...
  if(0!=pci_find_device(0x14f1,0x8800,0))
   CPK(printk("bttv doesn't support your Conexant 2388x card.\n"));
 }
 return rc;
}
