/* $Id: tda7432.c,v 1.13 2004/01/29 09:10:16 smilcke Exp $ */

/*
 * tda7432.c
 * Autor:               Stefan Milcke
 * Erstellt am:         06.12.2001
...
MODULE_PARM_LIST_HEAD(tda7432_parms)
MODULE_PARM(loudness,"i")
MODULE_PARM(maxvol,"i")
MODULE_PARM_LIST_TAIL(tda7432_parms)
...
static int tda7432_command(struct i2c_client *client,unsigned int cmd,void *arg);
...
static int tda7432_write(struct i2c_client *client,int subaddr,int val)
{
 ...
 CPK(printk("tda7432: In tda7432_write\n"));
 CPK(printk("tda7432: Writing %d 0x%x\n",subaddr,val));
 buffer[0]=subaddr;
 buffer[1]=val;
 if(2!=i2c_master_send(client,(char*)buffer,2))
 {
  CPK(printk(KERN_WARNING "tda7432: I/O error, trying (write %d 0x%x)\n"
             ,subaddr,val));
  return -1;
 }
 return 0;
...
static int tda7432_read(struct i2c_client *client)
{
 ...
 CPK(printk("tda7432: In tda7432_read\n"));
 if(1!=i2c_master_recv(client,&buffer,1))
 {
  CPK(printk(KERN_WARNING "tda7432: I/O error, trying (read)\n"));
  return -1;
 }
 CPK(printk("tda7432: Read 0x%02x\n",buffer);
 return buffer;
}
#endif
...
{
 struct tda7432 *t=(struct tda7432*)i2c_get_clientdata(client);
 unsigned char buf[16];
 CPK(printk("tda7432: In tda7432_set\n"));
 ...
 buf[8]=t->rr;
 buf[9]=t->loud;
 if(10!=i2c_master_send(client,(char*)buf,10))
 {
  CPK(printk(KERN_WARNING "tda7432: I/O error, trying tda7432_set\n"));
  return -1;
 }
 return 0;
}
...
{
 struct tda7432 *t=(struct tda7432*)client->data;
 CPK(printk("tda7432: In tda7432_init\n"));
                          ...
                          ,unsigned short flags,int kind)
{
 ...
 CPK(printk("tda7432: In tda7432_attach\n"));
 t=(struct tda7432*)kmalloc(sizeof(struct tda7432),GFP_KERNEL);
 if(!t)
  return -ENOMEM;
 memset(t,0,sizeof(struct tda7432));
 client=&t->c;
 ...
 CPK(printk(KERN_INFO "tda7432: init\n"));
...
{
 struct tda7432 *t=(struct tda7432*)i2c_get_clientdata(client);
 CPK(printk("tda7432: In tda7432 command\n"));
 switch(cmd)
 {
  // v4l ioctls
  // take care: bttv does userspace copying, we'll get a kernel pointer here...

  // Query card - scale from TDA7432 settings to V4L settings
  case VIDIOCGAUDIO:
   {
    struct video_audio *va=(struct video_audio*)arg;
    CPK(printk("tda7432: VIDIOCGAUDIO\n"));
  ...
  case VIDIOCSAUDIO:
   {
    struct video_audio *va=(struct video_audio*)arg;
    CPK(printk("tda7432: VIDIOCSAUDIO\n"));
    if(va->flags&VIDEO_AUDIO_VOLUME)
    {
     if(!maxvol)
      ...
      // max 0db
      t->volume=0x6f-((int)(va->volume)/829);
...
int tda7432_init(void)
{
 ...
 {
  CPK(printk(KERN_ERR "tda7432: loudness parameter must be between 0 and 15\n"));
  return -EINVAL;
 }
#ifdef TARGET_OS2
